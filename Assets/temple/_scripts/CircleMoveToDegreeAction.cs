...
public class CircleMoveToDegreeAction : ScriptAction
{
    ...
    protected override void UpdateAction()
    {
        ...
        else if (!moveOnCircleComplete)
        {
            // get current degree
            var angle = getAngle(center, actor);
            var targetAngle = (convertAngleFromUnity((targetDegree * Mathf.Deg2Rad)) + Mathf.PI) % (Mathf.PI * 2);
    ...
    {
        var targetAngle = targetDegree * Mathf.Deg2Rad;
        actor.transform.position = new Vector3(Mathf.Cos(targetAngle + Mathf.PI) * radius + center.transform.position.x, actor.transform.position.y, Mathf.Sin(targetAngle + Mathf.PI) * radius + center.transform.position.z);
