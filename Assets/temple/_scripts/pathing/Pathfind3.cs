...
namespace path3
{
    ...
    public class Position
    {
        ...
        public Position(float x, float z, int xIndex, int zIndex)
        {
            this.x = x;
            this.z = z;
            this.xIndex = xIndex;
    ...
    public class PositionGrid
    {
        ...
        public Position getPositionByIndex(int xIndex, int zIndex)
        {
            ...
            Position p = new Position(
                xIndex * unitSize - (xPositions - 1) * unitSize * 0.5f,
                zIndex * unitSize - (zPositions - 1) * unitSize * 0.5f,
                xIndex,
                zIndex
                );
        ...
        private bool attemptPath(int direction, Position parent, Position end, List<Position> unexplored)
        {
            ...
            // simple distance
            //float percent = (xSizeReal * 0.5f - distance) / (xSizeReal * 0.5f);

            float percent = Mathf.Min(preferred, Mathf.Abs(preferred - distance)) / preferred;
            float baseCost = parent.getMoveCost(direction);

            float avoidCost = percent * percent * 50f;
            //avoidCost = 0;
            ...
            {
                parent = parent,
                gCost = parent.pathInfo.gCost + baseCost + baseCost * avoidCost,
    ...
    public class Pathfind3 : MonoBehaviour {
	    ...
	    void Start () {
            grid = new PositionGrid(12, 12, 0.25f);
        }
        ...
        void Update()
        {
            ...
            if (path == null) calculatePath(0, 5);
            //if (path == null) calculatePath(5, 2);
